...
def create_employees_list_keyboard():
    """Создает клавиатуру со списком сотрудников для отметки"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
...
def handle_messages(message):
    ...
    # Если пользователь в процессе добавления сотрудника или отметки
    ...
    elif text == "✅ Отметить за сотрудника":
        if is_admin(user_id):
            mark_attendance_for_employee(message)
        else:
            bot.send_message(chat_id, "❌ У вас нет прав для этой операции")
    ...
    # Обработка отметки за сотрудника (по имени из списка)
    elif text.startswith("👤 "):
        if is_admin(user_id):
            employee_name = text[2:]  # Убираем эмодзи
            mark_specific_employee(message, employee_name)
        else:
            bot.send_message(chat_id, "❌ У вас нет прав для этой операции")
...
def handle_user_state(message):
    ...
    if text == "🔙 Назад":
        del user_states[user_id]
        if state == 'waiting_mark_employee':
            show_management_menu(chat_id)
        else:
            show_add_employee_menu(chat_id)
        return
...
def mark_attendance_for_employee(message):
    """Показывает список сотрудников для отметки"""
    user_id = message.from_user.id
    chat_id = message.chat.id
    ...
    bot.send_message(chat_id,
        ...
        reply_markup=create_employees_list_keyboard()
    )

def mark_specific_employee(message, employee_name):
    """Отмечает присутствие для конкретного сотрудника"""
    user_id = message.from_user.id
    chat_id = message.chat.id
    ...
    if cursor.fetchone():
        bot.send_message(chat_id, f"✅ {employee_name} уже отмечен сегодня!")
    else:
        cursor.execute('INSERT INTO attendance (employee_id, check_date) VALUES (?, ?)', (employee_id, today))
        conn.commit()
        bot.send_message(chat_id, f"✅ Присутствие отмечено для {employee_name}!")
    
    conn.close()
    
    # Очищаем состояние и возвращаем в меню управления
    if user_id in user_states:
        del user_states[user_id]
    show_management_menu(chat_id)
...
• Отметка присутствия за сотрудников
...
if __name__ == '__main__':
    init_db()
    logger.info("🤖 Бот запущен с функцией отметки за сотрудников!")
