    ...
    try:
        with conn.cursor() as cursor:
            ...
            if fetch:
                ...
                logger.info(f"✅ Запрос выполнен (fetch): {query[:50]}...")
            else:
                conn.commit()
                result = True
...
def create_employees_keyboard():
    ...
    for emp in employees:
        # ИСПРАВЛЕНИЕ: emp[0] - id, emp[1] - full_name
        keyboard.add(f"👤 {emp[1]}")  # Используем полное имя
...
def show_employee_report(message, employee_name):
    ...
    # ИСПРАВЛЕНИЕ: правильное обращение к элементам кортежа
...
def show_period_report(message, start_date, end_date):
    ...
    logger.info(f"✅ Показан отчет за период: {start_date} - {end_date}")
