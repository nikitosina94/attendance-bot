...
# Добавляем простую команду для добавления тестового сотрудника
@bot.message_handler(commands=['add_test'])
def add_test_employee(message):
    if not is_admin(message.from_user.id):
        return
    
    result = execute_query(
        'INSERT INTO employees (full_name, position) VALUES (%s, %s) RETURNING id',
        ('Тестовый Сотрудник', 'Тестовая должность'),
        fetch=True
    )
    
    if result:
        bot.send_message(message.chat.id, f"✅ Тестовый сотрудник добавлен (ID: {result[0][0]})")
    else:
        bot.send_message(message.chat.id, "❌ Ошибка добавления тестового сотрудника")

if __name__ == '__main__':
    logger.info("🔄 Запуск бота с улучшенным подключением к PostgreSQL...")
