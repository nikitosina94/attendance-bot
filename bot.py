...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effectivse_user.id
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "check_in":
        ...
        await query.edit_message_text("Отправьте Telegram ID сотрудника:")
...
async def add_employee_menu(query, context):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text("👥 Добавление сотрудника:", reply_markup=reply_markup)
...
async def check_in(query, context):
    ...
    if cursor.fetchone():
        await query.edit_message_text("✅ Вы уже отметили присутствие сегодня!")
    else:
...
async def manage_employees(query, context):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text("👥 Управление сотрудниками:", reply_markup=reply_markup)
    ...
    if context.user_data.get('waiting_for_telegram_id') and is_admin(user_id):
        try:
            ...
            if cursor.fetchone():
                ...
                conn.commit()
                await update.message.reply_text(f"✅ Сотрудник с Telegram ID {telegram_id} добавлен!")
