...
def init_db():
    ...
    try:
        ...
        if admin_id:
            cursor.execute('INSERT INTO admins (user_id) VALUES (%s) ON CONFLICT (user_id) DO NOTHING', (int(admin_id),))
...
def create_employees_keyboard():
    ...
    try:
        cursor.execute('SELECT id, full_name FROM employees WHERE is_active = TRUE ORDER BY full_name')
...
def handle_user_state(message):
    ...
    if state == 'waiting_employee_name':
        ...
        cursor = conn.cursor()
        try:
            cursor.execute('INSERT INTO employees (full_name, position) VALUES (%s, %s)', (employee_name, position))
            conn.commit()
...
def process_date_marking(message, date_str):
    ...
    try:
        ...
        try:
            ...
            cursor.execute('SELECT id FROM employees WHERE full_name = %s AND is_active = TRUE', (employee_name,))
            ...
            cursor.execute('SELECT * FROM attendance WHERE employee_id = %s AND check_date = %s', 
            ...
            if cursor.fetchone():
                ...
                cursor.execute('INSERT INTO attendance (employee_id, check_date) VALUES (%s, %s)', 
                              (employee_id, mark_date))
                conn.commit()
                bot.send_message(chat_id, 
            ...
            bot.send_message(chat_id, "❌ Ошибка при отметке присутствия")
        finally:
            cursor.close()
            conn.close()
...
def general_report(message):
    ...
    try:
        ...
        logger.error(f"Ошибка формирования общего отчета: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка при формировании отчета")
    finally:
        cursor.close()
        conn.close()
...
def show_employee_report(message, employee_name):
    ...
    try:
        # Находим сотрудника
        cursor.execute('SELECT id, position FROM employees WHERE full_name = %s', (employee_name,))
        employee = cursor.fetchone()
        ...
        logger.error(f"Ошибка формирования отчета по сотруднику: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка при формировании отчета")
    finally:
        cursor.close()
        conn.close()
...
def show_period_report(message, start_date, end_date):
    ...
    try:
        ...
        logger.error(f"Ошибка формирования отчета за период: {e}")
        bot.send_message(message.chat.id, "❌ Ошибка при формировании отчета")
    finally:
        cursor.close()
        conn.close()
...
if __name__ == '__main__':
    init_db()
    logger.info("🤖 Бот запущен с PostgreSQL и нижним меню!")
