...
def fix_database_url(url):
    ...
    if url and '-a/' in url and '.render.com' not in url:
        return url.replace('-a/', '-a.oregon-postgres.render.com/')
    return url
...
def show_employee_report(message, employee_name):
    ...
    for record in attendance_records[:15]:  # Показываем последние 15 записей
...
def main():
    ...
    logger.info("🚀 ЗАПУСК БОТА С ПОЛНЫМ ФУНКЦИОНАЛОМ...")
